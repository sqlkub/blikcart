...
export default function Header() {
  ...
  return (
    <header style={{ background: 'var(--navy)', color: 'white', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 8px rgba(0,0,0,0.2)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none', flexShrink: 0 }}>
          ...
          <span style={{ fontSize: 10, color: 'var(--gold)', textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 600 }}>Premium Saddlery</span>
        </Link>
        ...
        <nav style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          {NAV.map(item => (
            ...
            >
              <Link href={item.href} style={{
                ...
                background: openMenu === item.label ? 'rgba(255,255,255,0.1)' : 'transparent', transition: 'all 0.15s',
              }}>
                {item.label}
                        ...
                        <div>
                          ...
                          {child.children.map((gc: any) => (
                            <Link key={gc.label} href={gc.href} style={{ display: 'block', padding: '7px 16px 7px 24px', fontSize: 14, color: '#374151', textDecoration: 'none' }}
                              onMouseEnter={e => { (e.target as HTMLElement).style.background = 'var(--cream)'; (e.target as HTMLElement).style.color = 'var(--navy)'; }}
          ...
          <Link href="/customize" style={{ padding: '8px 14px', borderRadius: 6, fontSize: 14, fontWeight: 700, color: 'var(--gold)', textDecoration: 'none', border: '1px solid rgba(200,134,10,0.4)', marginLeft: 4 }}>
            Design Your Own ✦
          </Link>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <button onClick={() => toggleCart()} style={{ position: 'relative', padding: '6px 10px', background: 'none', border: 'none', color: 'rgba(255,255,255,0.85)', cursor: 'pointer', fontSize: 18, borderRadius: 6 }}>
            🛒
            {itemCount > 0 && <span style={{ position: 'absolute', top: 2, right: 2, background: 'var(--gold)', color: 'white', borderRadius: '50%', width: 16, height: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700 }}>{itemCount}</span>}
          </button>
        </div>
      </div>
    </header>
