...
const BASE_PRICES = [
  { category: 'Bridles', icon: '🐴', from: 38, to: 140, moq: 1, note: 'Full leather, padded, or simple strap styles', leadTime: '10–14 days' },
  { category: 'Browbands', icon: '💎', from: 18, to: 85, moq: 1, note: 'Plain leather, crystal-set, or embroidered', leadTime: '7–10 days' },
  { category: 'Saddle Pads', icon: '🪡', from: 24, to: 110, moq: 5, note: 'GP, dressage, or jumping cut', leadTime: '7–10 days' },
  { category: 'Horse Rugs', icon: '🧥', from: 65, to: 210, moq: 3, note: 'Turnout (lightweight to 400g) and stable', leadTime: '14–21 days' },
  { category: 'Head Collars', icon: '🔖', from: 16, to: 60, moq: 1, note: 'Leather or nylon with name plate option', leadTime: '7–12 days' },
  { category: 'Numnahs', icon: '🪢', from: 20, to: 75, moq: 5, note: 'Quilted and fleece-lined options', leadTime: '7–10 days' },
  { category: 'Leg Boots', icon: '🥾', from: 28, to: 95, moq: 4, note: 'Brushing, tendon, and over-reach styles', leadTime: '10–14 days' },
];
    ...
    <main style={{ minHeight: '100vh', background: '#faf9f7' }}>
      ...
      <section style={{ maxWidth: 1060, margin: '0 auto', padding: 'clamp(40px, 5vw, 64px) 24px' }}>
        ...
        <p style={{ fontSize: 14, color: '#666', marginBottom: 24 }}>
          Prices shown are base prices before option modifiers (hardware, material, embroidery, etc.) and exclude VAT.
          Final price is always shown live in the configurator before you submit.
        </p>
            ...
            <tbody>
              {BASE_PRICES.map((r, i) => (
                <tr key={r.category} style={{ background: i % 2 === 1 ? '#faf9f7' : '#fff' }}>
                  <td style={{ padding: '13px 16px', fontWeight: 700, color: '#1a1a1a', fontSize: 14 }}>
                    <span style={{ marginRight: 8 }}>{r.icon}</span>{r.category}
                  </td>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 48 }}>
          {VOLUME_TIERS.map(t => (
            <div key={t.range} style={{
              background: t.highlight ? '#fff8ec' : '#fff',
              border: `1.5px solid ${t.highlight ? '#C8860A' : '#e8e4de'}`,
              borderRadius: 12, padding: '20px 16px', textAlign: 'center',
              position: 'relative',
            }}>
              {t.highlight && (
                <span style={{ position: 'absolute', top: -10, left: '50%', transform: 'translateX(-50%)', background: '#C8860A', color: '#fff', fontSize: 10, fontWeight: 700, padding: '2px 10px', borderRadius: 20, whiteSpace: 'nowrap' }}>
                  Most Popular
                </span>
              )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: 40, alignItems: 'center' }}>
            <div>
              ...
              <p style={{ color: '#888', fontSize: 14, lineHeight: 1.75, marginBottom: 16 }}>
                Our full PDF price list includes per-unit pricing for every option combination across all 7 categories, with wholesale discount tiers applied.
                Available to approved wholesale partners and verified businesses.
              </p>
