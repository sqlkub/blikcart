...
const DEFAULT = {
  hero: {
    ...
    title: 'Price Lists',
    subtitle: 'Starting prices, volume discount tiers, and lead times for every product category. Live prices with all options are shown in the configurator.',
  },
  basePrices: [
  ...
  wholesaleSection: {
    title: 'Full Wholesale Price List',
    body: 'Our full PDF price list includes per-unit pricing for every option combination across all 7 categories, with wholesale discount tiers applied. Available to approved wholesale partners and verified businesses.',
    promoText: 'Alternatively, use our live configurator for instant per-unit pricing.',
  },
};
...
export default function PriceListsPage() {
  ...
  useEffect(() => {
    fetch(`${API}/content/pages/price-lists`)
      ...
      <section style={{ maxWidth: 1060, margin: '0 auto', padding: 'clamp(40px, 5vw, 64px) 24px' }}>
        ...
        <div style={{ background: '#fff', border: '1.5px solid #e8e4de', borderRadius: 14, overflow: 'hidden', marginBottom: 48 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {content.basePrices.map((r: any, i: number) => (
        ...
        <div style={{ background: '#fff', border: '1.5px solid #e8e4de', borderRadius: 14, padding: 'clamp(20px, 3vw, 32px)', marginBottom: 48 }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: '#1a1a1a', marginBottom: 8 }}>{content.expressSection.title}</h3>
          <p style={{ fontSize: 14, color: '#555', lineHeight: 1.75 }}>{content.expressSection.body}</p>
        </div>
        ...
        <div style={{ background: '#1a1a1a', borderRadius: 16, padding: 'clamp(28px, 4vw, 44px)', color: '#fff' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: 40, alignItems: 'center' }}>
            <div>
              <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 12 }}>{content.wholesaleSection.title}</h2>
              <p style={{ color: '#888', fontSize: 14, lineHeight: 1.75, marginBottom: 16 }}>
                {content.wholesaleSection.body}
              </p>
              <p style={{ color: '#C8860A', fontSize: 13, fontWeight: 600 }}>{content.wholesaleSection.promoText}</p>
            </div>
