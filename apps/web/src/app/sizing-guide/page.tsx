...
const thStyle: React.CSSProperties = {
  ...
  color: '#888', textTransform: 'uppercase' as const, letterSpacing: '0.06em',
  background: '#f5f3ef', borderBottom: '1px solid #e8e4de',
};
      ...
      <section style={{ maxWidth: 960, margin: '0 auto', padding: 'clamp(40px, 5vw, 64px) 24px' }}>
        ...
        <div id="bridles-browbands" style={{ marginBottom: 56 }}>
          ...
          <div style={{ background: '#fff', border: '1.5px solid #e8e4de', borderRadius: 14, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Size', 'Headpiece', 'Browband', 'Noseband', 'Cheekpieces'].map(h => (
        ...
        <div id="horse-rugs" style={{ marginBottom: 56 }}>
          ...
          <div style={{ background: '#fff', border: '1.5px solid #e8e4de', borderRadius: 14, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Size (ft)', 'Size (cm)', 'Back length', 'Chest width', 'Typical breed'].map(h => (
                    <th key={h} style={thStyle}>{h}</th>
                  ))}
                </tr>
              </thead>
        ...
        <div id="leg-boots" style={{ marginBottom: 56 }}>
          ...
          <div style={{ background: '#fff', border: '1.5px solid #e8e4de', borderRadius: 14, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Size', 'Cannon circumference', 'Typical fit'].map(h => (
                    <th key={h} style={thStyle}>{h}</th>
                  ))}
                </tr>
              </thead>
        ...
        <div id="how-to-measure" style={{ background: '#fff', border: '1.5px solid #e8e4de', borderRadius: 16, padding: 'clamp(24px, 3vw, 36px)', marginBottom: 32 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 20 }}>
            {[
              { icon: '🐴', title: 'Headpiece / Bridle', steps: ['Use a soft tape measure', 'Measure over the poll from cheek ring to cheek ring', 'Add 4 cm for buckle adjustment range'] },
              { icon: '🧥', title: 'Rug Back Length', steps: ['Start from the centre of the chest', 'Measure along the back following the spine', 'End at the base of the tail', 'Do not measure under the belly'] },
              { icon: '🥾', title: 'Cannon Bone', steps: ['Have the horse standing square', 'Use a soft tape around the cannon', 'Measure at the widest point (middle third)', 'Front and hind cannons may differ'] },
            ].map(m => (
              <div key={m.title}>
                <p style={{ fontSize: 15, fontWeight: 700, color: '#1a1a1a', marginBottom: 10 }}>{m.icon} {m.title}</p>
