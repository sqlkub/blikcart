...
const FEATURES = [
  { icon: '💼', title: 'Dedicated Account Manager', desc: 'One named contact for all your orders, quotes, and questions. Available Mon–Fri 09:00–17:00 CET.' },
  { icon: '💰', title: 'Volume Pricing', desc: 'Automatic discounts from 5 units. Custom pricing available for regular high-volume partners.' },
  { icon: '🏷️', title: 'Private Label & White Label', desc: 'Custom branding, swing tags, and packaging with your logo. Apply for branding requirements at time of ordering.' },
  { icon: '📋', title: 'Net-30 Payment Terms', desc: 'Approved accounts receive Net-30 invoicing after 3 completed orders. Apply after your first order.' },
  { icon: '📊', title: 'Order Management Portal', desc: 'Your account dashboard tracks every quote, order, and shipment across your entire purchase history.' },
  { icon: '🚚', title: 'Priority Production', desc: 'B2B accounts get priority scheduling in our production queue during peak seasons.' },
];
...
const STEPS = [
  ...
  { step: '02', title: 'Get Your Account Activated', desc: 'Once approved, you receive your B2B login, custom pricing tier, and your account manager\'s direct contact details.' },
  { step: '03', title: 'Place Orders via Configurator', desc: 'Log in and use the same step-by-step configurator — your wholesale prices are applied automatically.' },
  { step: '04', title: 'Manage Everything in One Place', desc: 'All quotes, orders, shipments, and invoices are accessible from your account dashboard.' },
];
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#faf9f7' }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, #1a1a1a 0%, #2d2017 100%)', color: '#fff', padding: 'clamp(56px, 7vw, 88px) 24px', textAlign: 'center' }}>
        <p style={{ fontSize: 12, letterSpacing: 4, textTransform: 'uppercase', color: '#C8860A', fontWeight: 700, marginBottom: 14 }}>Business Accounts</p>
        <h1 style={{ fontSize: 'clamp(28px, 4vw, 46px)', fontWeight: 800, margin: '0 0 16px', letterSpacing: '-0.02em' }}>B2B Portal</h1>
        <p style={{ fontSize: 16, color: '#aaa', maxWidth: 540, margin: '0 auto 28px', lineHeight: 1.7 }}>
          Unlock wholesale pricing, private-label options, Net-30 terms, and a dedicated account manager — all through one login.
        </p>
      ...
      <section style={{ background: '#fff', borderBottom: '1px solid #e8e4de' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))' }}>
          {[
            ...
            <div key={s.label} style={{ padding: '24px 16px', textAlign: 'center', borderRight: i < 3 ? '1px solid #e8e4de' : 'none' }}>
      ...
      <section style={{ maxWidth: 1060, margin: '0 auto', padding: 'clamp(48px, 6vw, 72px) 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16, marginBottom: 56 }}>
          {STEPS.map(s => (
            <div key={s.step} style={{ background: '#fff', border: '1.5px solid #e8e4de', borderRadius: 14, padding: '24px 20px' }}>
              ...
              {s.link && (
                <Link href={s.link} style={{ display: 'inline-block', marginTop: 12, fontSize: 13, fontWeight: 700, color: '#C8860A', textDecoration: 'none' }}>
                  {s.linkText} →
                </Link>
              )}
            </div>
        ...
        <div style={{ background: '#1a1a1a', borderRadius: 16, padding: 'clamp(28px, 4vw, 44px)', textAlign: 'center', color: '#fff' }}>
          <h3 style={{ fontSize: 22, fontWeight: 800, marginBottom: 10 }}>Ready to Partner?</h3>
          <p style={{ color: '#888', fontSize: 15, marginBottom: 24, maxWidth: 420, margin: '0 auto 24px' }}>Apply for a wholesale account in 2 minutes. We approve and respond within 1 business day.</p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/wholesale" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#C8860A', color: '#fff', padding: '13px 28px', borderRadius: 8, fontWeight: 700, fontSize: 15, textDecoration: 'none' }}>
              Apply for Wholesale <ArrowRight size={16} />
            </Link>
            <a href="mailto:wholesale@blikcart.nl" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.85)', padding: '12px 20px', borderRadius: 8, fontWeight: 600, fontSize: 14, textDecoration: 'none', border: '1px solid rgba(255,255,255,0.12)' }}>
              wholesale@blikcart.nl
            </a>
          </div>
        </div>
      </section>
