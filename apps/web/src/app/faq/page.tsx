...
const FAQS = [
  {
    ...
    items: [
      ...
      { q: 'Can I upload reference images or files?', a: 'Yes. The configurator\'s Notes & References step lets you upload images, PDFs, or other files (up to 20 MB each). Useful for colour references, logo artwork, or measurements.' },
    ],
  },
  {
        ...
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '16px 24px', display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
          {FAQS.map(cat => (
            <a key={cat.category} href={`#${cat.category.replace(/\s+&\s+|\s+/g, '-').toLowerCase()}`}
              style={{ fontSize: 13, fontWeight: 600, color: '#555', padding: '6px 14px', borderRadius: 20, background: '#f5f5f5', textDecoration: 'none', border: '1px solid #e8e4de', transition: 'all 0.15s' }}>
      ...
      <section style={{ maxWidth: 820, margin: '0 auto', padding: 'clamp(40px, 5vw, 64px) 24px' }}>
        {FAQS.map(cat => (
          <div key={cat.category} id={cat.category.replace(/\s+&\s+|\s+/g, '-').toLowerCase()} style={{ marginBottom: 48 }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: '#1a1a1a', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
